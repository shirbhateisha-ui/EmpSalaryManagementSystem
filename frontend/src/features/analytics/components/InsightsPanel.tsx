...
import type { CountryPayroll, DepartmentPayroll, SalaryBand, SalarySummary, TopEarner } from '../types/analytics.types';
...
export function InsightsPanel({ summary, byCountry, byDepartment, distribution, topEarners }: InsightData) {
    ...
    <Card>
      ...
      <CardContent className="p-0">
        <InsightRow question="What is our total monthly payroll spend?">
          <p className="text-2xl font-bold text-foreground">{USD.format(summary.total_monthly_usd)}<span className="text-base font-normal text-muted-foreground">/month</span></p>
          <p className="mt-1">Annual total: {USD.format(summary.total_annual_usd)} across {summary.headcount} employees.</p>
        </InsightRow>
        ...
        <InsightRow question="Which country has the highest payroll cost?">
          {highestCountry ? (
            <p>
              ...
              {' '}— {COMPACT.format(highestCountry.total_annual_usd)}/year
              ({highestCountry.headcount} employees, avg {USD.format(highestCountry.avg_annual_usd)}).
            </p>
        ...
        <InsightRow question="Which department has the highest total payroll?">
          {highestDept ? (
            <p>
              ...
              {' '}— {COMPACT.format(highestDept.total_annual_usd)}/year
              ({highestDept.headcount} employees).
            </p>
          ) : <p>No data available.</p>}
        </InsightRow>
            ...
            <ol className="space-y-1 mt-1">
              {topEarners.slice(0, 10).map((e, i) => (
                <li key={e.id} className="flex justify-between">
                  <span><span className="text-muted-foreground mr-2">{i + 1}.</span>{e.name} <span className="text-muted-foreground">({e.department})</span></span>
